...
class PaymentController extends Controller
{
    ...
    public function store(Request $request, PaymentService $service): JsonResponse
    {
        $data = $request->validate([
            ...
            'method' => ['required', 'in:mpesa,bank,cash,card,manual'],
            'amount' => ['required', 'numeric', 'gt:0'],
            'currency' => ['sometimes', 'string', 'size:3'],
            'external_reference' => ['nullable', 'string', 'max:120'],
            'transaction_reference' => ['nullable', 'string', 'max:120', 'unique:payments,transaction_reference'],
    ...
    public function allocate(Request $request, Payment $payment, PaymentService $service): JsonResponse
    {
        $data = $request->validate(['invoice_id' => ['required', 'integer', 'exists:invoices,id'], 'amount' => ['required', 'numeric', 'gt:0']]);
        $result = DB::transaction(fn () => $service->allocate($payment, Invoice::findOrFail($data['invoice_id']), (float) $data['amount']));
